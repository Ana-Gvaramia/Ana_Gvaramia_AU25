...
BEGIN
    ...
    SELECT c.name AS category,
           SUM(p.amount)::DECIMAL AS total_sales_revenue
    FROM
        public.payment p
        INNER JOIN public.rental r ON p.rental_id = r.rental_id
        INNER JOIN public.inventory i ON r.inventory_id = i.inventory_id
...
BEGIN
    ...
    WITH film_rentals_by_country AS (
        ...
        WHERE
            co.country ILIKE ANY(country_names)
        GROUP BY
...
BEGIN
    ...
    WITH film_inventory_status AS (
        SELECT
            ...
            l.name AS language,
            'Available'::TEXT AS customer_name,
    ...
    SELECT
        ROW_NUMBER() OVER (ORDER BY fis.film_title, fis.customer_name) AS row_num,
        fis.film_title,
        fis.language,
        fis.customer_name,
        fis.rental_date
